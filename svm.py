...
def svm(file_dataframe, test_size=0.2, cols=['date', 'open']):
    ...
    if 'date' in file_dataframe:
        file_dataframe['new_col'] = pd.to_datetime(
            file_dataframe['date']).astype(datetime)
        # file_dataframe['date'] = pd.to_datetime(file_dataframe['date'])
        file_dataframe['new_col'].apply(
            lambda dt_time: 10000*dt_time.year + 1000*dt_time.month + dt_time.day).astype(int)
...
def main(dir_path):
    ...
    for file_name in files:
        ...
        file_dataframe = pd.read_csv(os.path.join(
            dir_path, file_name), parse_dates=[1])
