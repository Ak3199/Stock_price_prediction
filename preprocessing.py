        ...
        with open(os.path.join(dir_path, file_name), 'r') as textfile:
            ...
            for count, row in enumerate(reversed(list(csv.reader(textfile)))):
                ...
                if count < num_moving_avg:
                    ...
                    avg = roundup((num_moving_avg * avg + float(row[7]) - float(
                        new_list[count - num_moving_avg][7])) / (num_moving_avg))

                prev = float(row[7])

                if count < num_volatile:
                    volatile_avg = roundup(
                        (count * volatile_avg + float(row[7])) / (count + 1))
                else:
                    volatile_avg = roundup((num_volatile * volatile_avg + float(
                        row[7]) - float(new_list[count - num_volatile][7])) / (num_volatile))

                if count:
                    loop_count = min(count, num_volatile)

                    for i in range(loop_count):
                        curr_volatility += math.pow(
                            (float(row[7]) - volatile_avg), 2)

                    curr_volatility = roundup(
                        math.sqrt(curr_volatility / (loop_count)))
            ...
            new_list.insert(0, ['symbol', 'date', 'open', 'high', 'low', 'close', 'volume',
